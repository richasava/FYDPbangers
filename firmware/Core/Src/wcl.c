...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define MPU_ADDR      (0x68 << 1)   // HAL uses 8-bit addresses
...
/* USART1 is owned entirely by the WCL module (see Core/Src/wcl.c), which
 * configures it at 921600 baud for the COBS+protobuf bridge to the ESP32.
 * Do not add a second handle for it here. */
...
/**
 * @brief redirects MPU printing to usart 2, idk default for now
 */
...
int main(void)
{
  ...
  uint8_t who = 0;
  HAL_I2C_Mem_Read(&hi2c1, MPU_ADDR, REG_WHO_AM_I, 1, &who, 1, 100);
  printf("WHO_AM_I = 0x%02X (expect 0x68)\r\n", who);

  if (who == 0x68) {
      uint8_t wake = 0x00; // clear sleep bit
      HAL_I2C_Mem_Write(&hi2c1, MPU_ADDR, REG_PWR_MGMT1, 1, &wake, 1, 100);
      printf("MPU awake, streaming...\r\n");
  } else {
      printf("MPU not found - check wiring/pullups\r\n");
  }

  /* Bring up the WCL bridge (USART1 @ 921600, COBS + protobuf -> ESP32). */
  if (!WCL_Init()) {
      printf("WCL init FAILED\r\n");
  } else {
      printf("WCL up: USART1 @ 921600, COBS+protobuf\r\n");
  }
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // yaw needs gyro integration + startup bias estimate, not done yet.
	  // hr/gsr channels are not live, so their valid flags stay false.
	  if (!WCL_SendBiometricFrame(roll, pitch, 0.0f, true,   /* imu live */
	                              0.0f, false,               /* hr  not yet */
	                              0.0f, false)) {            /* gsr not yet */
	      printf("WCL tx fail\r\n");
	  }

	  printf("roll=%6.1f  pitch=%6.1f\r\n", roll, pitch);
	  HAL_Delay(10);   /* ~100 Hz placeholder; replace with TIM2 1 kHz tick (N2) */
  }
  /* USER CODE END 3 */
}
